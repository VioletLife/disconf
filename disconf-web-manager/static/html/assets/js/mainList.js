(function ($) {
  ...
  function fetchVersion (appId, envId) {
    var base_url = serverAPIUrl + 'api/web/config/versionlist?appId=' + appId
    url = base_url
    if (envId != -1) {
  ...
  function fetchMainList () {
    ...
    $('#batch_download').attr(
            'href',
          serverAPIUrl + 'api/web/config/downloadfilebatch?appId=' + appId + '&envId=' +
                envId + '&version=' + version)

    $('#mainlist_error').hide()
    var parameter = ''

    url = serverAPIUrl + 'api/web/config/list'
  ...
  function fetchConfigValue (configId, object) {
    ...
    $.ajax({
      type: 'GET',
      url: serverAPIUrl + 'api/web/config/' + configId
    }).done(
            function (data) {
              if (data.success === 'true') {
  ...
  function fetchZkInfo (configId, object) {
    ...
    $.ajax({
      type: 'GET',
      url: serverAPIUrl + 'api/web/config/zk/' + configId
    }).done(
            function (data) {
              if (data.success === 'true') {
                var result = data.result
  ...
  function deleteDetailTable (id, name) {
    ...
    $.ajax({
      type: 'DELETE',
      url: serverAPIUrl + 'api/web/config/' + id
    }).done(function (data) {
      if (data.success === 'true') {
        fetchMainList()
      }
  ...
  function fetchZkDeployInfo () {
    ...
    var base_url = serverAPIUrl + 'api/zoo/zkdeploy?appId=' + appId + '&envId=' + envId +
            '&version=' + version
